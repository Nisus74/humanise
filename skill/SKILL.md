...
- The two or three `profile/sample-<channel>-*.md` files closest to the current channel, read raw. The fingerprint is the map; the samples are the territory, and imitation of the territory is what produces the voice. If the channel has no direct samples, take the nearest channel's samples, note the gap to the user, and lean harder on the fingerprint. For long-form documents (memos, board papers, PRDs), read whichever long-form exemplars exist (`profile/sample-long-form-*.md`, plus the closest formal-channel samples such as `profile/sample-board-paper-*.md`, `profile/sample-prd-*.md`, `profile/sample-investor-update-*.md`); when none exist yet, lean on `profile/register-descriptors.md`, which ships in the profile template to capture document-level habits before the corpus does. The chat samples don't tell you how the user builds a document.
...
Pass one has two parts. **Part A (automated):** the script covers checklist items 1 to 5 and 8 to 9 below, plus slop openers and the spaced-hyphen dash, and most of pass two. Fix every failure it reports and rerun until Part A is clean. **Part B (manual):** items 6 and 7 (the -ing phrase chains and the formatting tells) are not in the script; scan for them by eye once Part A is clean. Run the script rather than eyeballing Part A: the model's self-assessment is unreliable on exactly these items, and the attention saved goes to the voice work and to Part B, where no script helps.
