...
Turns your writing samples into `profile/voice-fingerprint.md`, the distilled model the skill reads on every draft. Run it after adding samples to `profile/voice-corpus/`, and again whenever you add more.
...
- Every sample in `profile/voice-corpus/**` and its annotations.
...
3. **Measure the tripwires.** Run `evals/assertions/writing_checks.py` on two or three samples and record the ranges (sentence-length spread, contraction types, dialect markers, burstiness). These become the diagnostic table, as tripwires not targets.
...
- **Opening moves** and **closing moves:** the shapes that recur, with examples.
...
- **Diagnostic profile:** the measured tripwire table.
