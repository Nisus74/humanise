...
5. Collect 5 to 10 writing samples with `scripts/corpus-questionnaire.md` (it gives a concrete fetch prompt per channel and fills the annotations by asking); save them to `profile/voice-corpus/` per channel. Then run `scripts/generate-fingerprint.md` to write `profile/voice-fingerprint.md` and build the voiceprint baseline.
